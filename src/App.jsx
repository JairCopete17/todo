...
export default function App () {
  const localtasks = JSON.parse(localStorage.getItem('localtasks')) || [{ id: getUniqueId(), text: 'Start here', state: false }]

  const [tasks, setTasks] = useState(localtasks)

  const addTask = (text) => {
    const newTask = { id: getUniqueId(), text: text, state: false }
